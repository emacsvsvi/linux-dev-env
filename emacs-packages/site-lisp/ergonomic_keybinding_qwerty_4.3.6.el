...
;; Note: Unless otherwise specified, then, in the following, QWERTY layout is used to indicate letter positions. So, if it say “Changed M+y to something”, that “y” means the key position on qwerty (which is Dvorak's “t”).

;; version 4.3.6, 2009-05-26 • dired's M-s changed in emacs 23, such that M-s is now a prefix. For Dvorak users, M-s should be isearch. This is reclaimed to be isearch.
...
;; Move by paragraph
;;  Stan - Commented out the next two lines because one of them causes the arrow keys to stop working(in linux), and I don't use these functions.
...
(add-hook 'w3m-mode-hook
 (lambda ()
  (define-key w3m-mode-map (kbd "<up>") 'next-line) ; was w3m-previous-anchor. Use Shift+Tab.
  (define-key w3m-mode-map (kbd "<down>") 'previous-line) ; was w3m-next-anchor. Use Tab.
